...
class GuiMocap :
	...
	def get_select(self) :
		...
		print(self.subject_id,self.PI_id,self.session_nb,self.target_id)
		self.set_directory(self.subject_id,self.PI_id,self.session_nb,self.target_id)
	...
	def display_GUI(self,sub_name='Entrer sujet',session_nb='1') :
		...
		# liste des cibles
		self.listeball = Listbox(f2,exportselection=False)
		self.listeball.insert(1, "Feather")
		self.listeball.insert(2, "Red")
		self.listeball.insert(3, "Empty")
		self.listeball.pack()
		
		# titres
		self.labelPI = Label(f5, text="Sélectionner PI",bg="grey")
		self.labelPI.pack()
		self.labelball = Label(f4, text="Sélectionner Cible",bg="grey")
		self.labelball.pack()
